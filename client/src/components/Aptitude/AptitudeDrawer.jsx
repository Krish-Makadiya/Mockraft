...
const answeredDemo = [
    ...
    true,
    true,
    ...Array(TOTAL_QUESTIONS - 5).fill(false),
];

const AptitudeDrawer = ({ currentQuestions, isOpen, setIsOpen }) => {
    const navigate = useNavigate();

    return (
        <div
            className={`fixed inset-0 overflow-hidden z-10 ${!isOpen ? "pointer-events-none" : ""}`}
        >
            {/* Backdrop */}
            <div
                className={`fixed inset-0 bg-light-secondary-text/90 dark:bg-gray-500/90 transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0"}`}
            ...
            <div className="fixed inset-y-0 right-0 flex max-w-full pl-10">
                ...
                >
                    {/* Close button */}
                    <div
                        className={`absolute top-0 left-0 -ml-8 pt-4 pr-2 sm:-ml-10 sm:pr-4 ${!isOpen ? "hidden" : ""}`}
                    >
                        <button
                            type="button"
                            onClick={() => setIsOpen(false)}
